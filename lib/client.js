...
SearchSource.prototype.getData = function(options, getCursor) {
  ...
  function transform(doc) {
    if (options.transform) {
      self.searchFields.forEach(function(field) {
        let propertyValue = _byString(doc, field);
        if (self.currentQuery && propertyValue) {
          _assign(doc, field, options.transform(propertyValue, regExp, field, self.currentQuery));
        }
      });
    }
